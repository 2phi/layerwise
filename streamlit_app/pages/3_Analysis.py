...
def _evaluate_system(system: SystemModel, criteria_evaluator: CriteriaEvaluator):
    ...
    DERR_crit = criteria_evaluator.fracture_toughness_envelope(
        DERR_I, DERR_II, system.weak_layer
    )
    IERR_crit = criteria_evaluator.fracture_toughness_envelope(
        IERR_I, IERR_II, system.weak_layer
    )
...
def update_segments(
    segments: List[Segment], crack_mid_point: float, crack_length: float
) -> List[Segment]:
    ...
    for segment in segments:
        ...
        # crack in the middle of the segment
            ...
            start_point < crack_mid_point - crack_length / 2
            and end_point > crack_mid_point + crack_length / 2
        ):
            new_segments.append(
                Segment(
                    length=crack_mid_point - crack_length / 2 - covered_length,
            ...
            new_segments.append(
                Segment(
                    ...
                    - (crack_mid_point + crack_length / 2 - covered_length),
                    has_foundation=segment.has_foundation,
                    m=segment.m,
                )
            )
...
def plot_system_evaluation_with_params(
    system: SystemModel, criteria_evaluator: CriteriaEvaluator, window_size: int
):
    ...
    for case_name, case_info in cases.items():
        try:
            ...
            (
                ...
                IERR_I,
                IERR_II,
            ) = _evaluate_system(current_system, criteria_evaluator)
    ...
    for case_name, case_info in cases.items():
        ...
        if np.any(mask_no_crack):
            ax.plot(
                x_orig[mask_no_crack],
                derr[mask_no_crack],
                color=case_info["color"],
                linewidth=2,
                linestyle="-",
                label=f"{case_info['label']} DERR Critical",
            )
            ax.plot(
                x_orig[mask_no_crack],
                ierr[mask_no_crack],
                ...
                linewidth=2,
                linestyle="--",
                label=f"{case_info['label']} IERR Critical",
            )
...
def plot_stress_envelope_comparison(
    selected_cases: List[str], criteria_evaluator: CriteriaEvaluator
):
    ...
    colors = {
        "current": "blue",
        "coupled_criterion": "red",
        "minimum_force": "green",
        "minimum_crack_length": "orange",
    }
    ...
    for case_name in selected_cases:
        ...
        ax.plot(
            ...
            label=f"{case_name.replace('_', ' ').title()} Stress Path",
        )
        ax.scatter(
            sigma[0], tau[0], color=colors[case_name], s=10, marker="o", alpha=0.7
        )
        ax.scatter(
            sigma[-1], tau[-1], color=colors[case_name], s=10, marker="s", alpha=0.7
        )
...
def plot_err_envelope_comparison(
    selected_cases: List[str], criteria_evaluator: CriteriaEvaluator
):
    ...
    for case_name in selected_cases:
        ...
        ax.scatter(
            ...
            s=50,
            marker="o",
            label=f"{case_name.replace('_', ' ').title()} Incremental ERR",
            alpha=0.7,
        )
        ax.scatter(
            np.abs(DERR_I),
            ...
            s=50,
            marker="s",
            label=f"{case_name.replace('_', ' ').title()} Differential ERR",
            alpha=0.7,
        )
    ...
    if selected_cases:
        ...
        if reference_system is not None:
            ...
            ax.text(
                0.01, G_IIc + 0.02, r"$G_{IIc}$", color="black", ha="left", va="center"
            )
            ...
            def find_GI_for_GII(GII_val):
                ...
                try:
                    GI_root = brentq(
                        envelope_root_func, a=0, b=50, xtol=1e-6, rtol=1e-6
                    )
                    return GI_root
                except ValueError:
                    return np.nan
...
if "system" not in st.session_state:
    st.warning(
        "Please assemble the slab and the scenario on the 'Slab Definition & Scenario Definition' page first."
    )
    st.stop()
...
params_changed = any(
    current_params[k] != st.session_state.params[k] for k in ["weight"]
)
window_changed = any(
    current_params[k] != st.session_state.params[k] for k in ["window_size"]
)
...
if needs_full_recompute:
    with st.spinner("Computing all analysis cases..."):
        ...
        cc_system.update_scenario(
            segments=coupled_criterion_result.final_system.scenario.segments
        )
...
if not window_changed or "analysis_plot" not in st.session_state:
    with st.spinner("Generating analysis plot..."):
        ...
        min_crack_length = st.session_state.computed_results["minimum_crack_length"][
            "crack_length"
        ]
        coupled_criterion_result = st.session_state.computed_results[
            "coupled_criterion"
        ]["result"]
        fig = plotter.plot_analysis(
            current_system,
            criteria_evaluator,
            min_force_result,
            min_crack_length,
            coupled_criterion_result,
            window=window_size,
        )
...
if not window_changed or "current_plot" not in st.session_state:
    ...
    with col2:
        with st.spinner("Generating plot..."):
            fig = plot_system_evaluation_with_params(
                current_system, criteria_evaluator, window_size
            )
...
if selected_cases:
    ...
    with tab1:
        with st.spinner("Generating stress envelope plot..."):
            fig_stress = plot_stress_envelope_comparison(
                selected_cases, criteria_evaluator
            )
...
if st.session_state.computed_results["coupled_criterion"] is not None:
    ...
    if cc_data is not None and "result" in cc_data:
        ...
        with col1:
            st.metric(
                "Coupled Criterion - Critical Weight",
                f"{cc_result.critical_skier_weight:.1f} kg",
            )
        with col2:
            st.metric(
                "Coupled Criterion - Crack Length", f"{cc_result.crack_length:.1f} mm"
            )
        with col3:
...
if st.session_state.computed_results["minimum_force"] is not None:
    ...
    if mf_data is not None and "result" in mf_data:
        ...
        with col1:
            st.metric(
                "Stress Criterion - Critical Weight",
                f"{mf_result.critical_skier_weight:.1f} kg",
            )
        with col2:
            pass
...
if st.session_state.computed_results["minimum_crack_length"] is not None:
    ...
    if (
        ...
        and "crack_length" in mc_result
        and "segments" in mc_result
    ):
